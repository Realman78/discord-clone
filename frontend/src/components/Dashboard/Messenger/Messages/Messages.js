  ...
  return (
    <MainContainer>
      ...
      {messages.map((message, index) =>{
        const sameAuthor = index > 0 && messages[index-1].author._id === message.author._id
        ...
        return (<div key={message._id} style={{width: '97%'}}>
          ...
          <Message content={message.content} username={message.author.username} sameAuthor={sameAuthor} date={convertDateToHumanReadable(new Date(message.date), 'dd/mm/yy')} sameDay={sameDay}/>
        </div>
        )
      })}
    </MainContainer>
